...
def place_order(order_type):
    ...
    res = client_testnet.futures_create_order(symbol=SYMBOL,
...
# 🔄 Trailing stop and trade management
def manage_trade():
...
def close_position(exit_price, reason):
    global in_position, target_hit, trailing_peak, trailing_stop_price, current_trail_percent
            ...
            if not in_position:
                ...
                if pending_order_id:
                    ...
                    if order['status'] == 'FILLED':
                        entry_price = float(order['avgFillPrice']) if float(order['avgFillPrice']) > 0 else float(order['stopPrice'])
                        in_position = True
                        trailing_peak, trailing_stop_price, current_trail_percent = entry_price, None, 0.005
